...
public class PrinterUtils {
   ...
   public static <T> void print(List<List<T>> list) {
      ...
      list.stream().map(l -> l.stream().map(Object::toString).collect(Collectors.joining(",")))
   ...
   private static void dfs(Node node, HashSet<Node> visited) {
      ...
      if (node == null)
         return;
      visited.add(node);
      System.out.println(node.val + ":[" + node.neighbors.stream().map(n -> n.val).map(Object::toString)
