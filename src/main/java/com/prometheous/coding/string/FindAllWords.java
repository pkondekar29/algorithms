...
public class FindAllWords {
   ...
   public static void main(String[] args) {
      ...
      findAll(board, new String[] { "SEE", "ABFDEE", "SSAHFALK" }).forEach(System.out::println);
   }
   ...
   private static boolean DFS(char[][] board, int bi, int bj, int si, String w) {
      ...
      if (si == w.length())
         return true;
      if (bi < 0 || bi >= board.length || bj < 0 || bj >= board[0].length || board[bi][bj] != w.charAt(si))
         return false;
      ...
      boolean found =
            DFS(board, bi + 1, bj, si + 1, w) || DFS(board, bi - 1, bj, si + 1, w) || DFS(board, bi, bj + 1, si + 1, w)
                  || DFS(board, bi, bj - 1, si + 1, w);
