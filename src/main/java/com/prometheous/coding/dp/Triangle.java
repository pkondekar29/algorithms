...
public class Triangle {
   ...
   private Integer dfs(List<List<Integer>> triangle, int level, int vertex, List<List<Integer>> memo) {
      ...
      if (level == triangle.size() - 1)
         return triangle.get(level).get(vertex);
      // memo look up
      if (vertex < memo.get(level).size() && memo.get(level).get(vertex) != null)
         return memo.get(level).get(vertex);

      int minPath = triangle.get(level).get(vertex);
      minPath = Math
            .min(minPath + dfs(triangle, level + 1, vertex, memo),   // Left subtree on next level and same vertex
                  minPath + dfs(triangle, level + 1, vertex + 1,
                        memo));   // Right subtree on next level and next vertex
   ...
   public int minimumTotalDP(List<List<Integer>> triangle) {
      ...
      for (int i = 1; i <= levels; i++) {
         for (int j = 0; j < levels - i; j++) {
            ...
            int minPathWeight = Math.min(dp.get(i - 1).get(j) + currWeight, dp.get(i - 1).get(j + 1) + currWeight);
   ...
   public int minimumTotalDPSpaceOptON(List<List<Integer>> triangle) {
      ...
      for (int i = levels - 2; i >= 0; i--) {
         ...
         for (int j = 0; j < levelSize; j++) {
            ...
            // Min path weight at j
            dp[j] = Math.min(dp[j] + currWeight, dp[j + 1] + currWeight);
         }
      }
      return dp[0];
   }
