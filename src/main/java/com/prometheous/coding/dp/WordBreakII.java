...
//        PrinterUtils.printList(new WordBreakII().wordBreakDP("catsanddog", Arrays.asList("cat","cats","and","sand","dog")));
        PrinterUtils.printList(new WordBreakII().wordBreakMemo("catsanddog", Arrays.asList("cat","cats","and","sand","dog")));
    }
            ...
            if(dp[i] != null) { // We can make some sentence till here
                for(String word : wordDict) {
                    ...
                    if(nextPotentialMatchIndex <= n && str.indexOf(word, i) == i) {
                        dp[nextPotentialMatchIndex] = dp[nextPotentialMatchIndex] == null ?
                                new ArrayList<>() : dp[nextPotentialMatchIndex];

                        dp[nextPotentialMatchIndex].addAll(dp[i]);
                        dp[nextPotentialMatchIndex] = dp[nextPotentialMatchIndex].stream().map(s -> s + " " + word).collect(Collectors.toList());
                    }
                }
            }
        }
        for(List<String> l : dp) if(l != null) PrinterUtils.printList(l);
        return dp[n];
    }
            ...
            if(str.startsWith(word, at)) {   // We can match the current word at index at
                ...
                if(possibleSentences != null && possibleSentences.size() > 0) {
                    for(String possibleSentenceAfterThis : possibleSentences) {
                        String possibleSentence = possibleSentenceAfterThis.length() == 0 ? word : word + " " + possibleSentenceAfterThis;
