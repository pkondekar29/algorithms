...
public class HouseRobber {
   ...
   private static int dfsLeft(int[] nums, int i, boolean startedFrom1stHouse, Integer[] memo) {
      ...
      if (memo[i] != null)
         return memo[i];
      int robbed = nums[i] + Math
            .max(dfsLeft(nums, i + 2, startedFrom1stHouse, memo), dfsLeft(nums, i + 3, startedFrom1stHouse, memo));
      memo[i] = robbed;
      return robbed;
   }
   ...
   private static int dfsRight(int[] nums, int i, boolean startedFrom1stHouse, Integer[] memo) {
      ...
      if (memo[i] != null)
         return memo[i];
      int robbed = nums[i] + Math
            .max(dfsRight(nums, i - 2, startedFrom1stHouse, memo), dfsRight(nums, i - 3, startedFrom1stHouse, memo));
      memo[i] = robbed;
      return robbed;
   }
