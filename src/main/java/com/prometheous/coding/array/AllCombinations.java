...
public class AllCombinations {
   ...
   private static void findAllCombinations(int n, int k, int counter, int currIndex, int[] arr,
   ...
   public static List<List<Integer>> find(int n, int k) {
      ...
      for (int i = 0; i < p; i++) {
         ...
         if (str.length() != n) {
            String zeros = IntStream.range(0, n - str.length()).mapToObj(x -> "0").collect(Collectors.joining());
            str = zeros + str;
         }
         int noOfOnes = 0;
